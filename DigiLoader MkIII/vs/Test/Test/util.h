...
namespace Util
{
	namespace Impl
	{
		...
		struct apply_impl
		{
			static auto call(F f, Tuple && t) -> decltype(apply_impl<F, Tuple, Total == 1 + sizeof...(N), Total, N..., sizeof...(N)>::call(f, std::forward<Tuple>(t)))
			{
		...
		void assign(T& p, std::vector<void*>& arr)
		{
		...
		void assign(T& p, int index, std::vector<void*>& arr, integer_sequence<Is...>)
		{
			typedef void(*FT)(T&, std::vector<void*>&);
			static const FT fa[] = { &assign<Is, T>... };
			fa[index](p, arr);
		}
	}
	...
	auto apply(F f, Tuple && t) -> decltype(Impl::apply_impl<F, Tuple, 0 == std::tuple_size<std::decay<Tuple>::type>::value, std::tuple_size<std::decay<Tuple>::type>::value>::call(f, std::forward<Tuple>(t)))
	{
		typedef typename std::decay<Tuple>::type ttype;
		return Impl::apply_impl<F, Tuple, 0 == std::tuple_size<ttype>::value, std::tuple_size<ttype>::value>::call(f, std::forward<Tuple>(t));
	}

	template<class T>
	void assign(T& p, int index, std::vector<void*>& arr)
	{
		Impl::assign(p, index, arr, Impl::gen_sequence<std::tuple_size<T>::value>{});
	}
